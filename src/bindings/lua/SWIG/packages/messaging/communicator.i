...
namespace algol {
  ...
  // %feature("notabstract") communicator;
  %feature("director") communicator;
   ...
   * channels.
   * Communicators and their targets must be uniquely identifiable per-network and not per-instance.
   *
   * A usage example:
   *
  ...
  public:
    ...
    %rename(publish) send(const message&, const channel_id_t&, const string_t &queue);
    /**
